...
export async function getTransactions(userId: string): Promise<Transaction[]> {
  ...
  const transactions = await TransactionModel.find({ userId }).sort({ date: -1, createdAt: -1 }).lean()
...
export async function updateLoan(userId: string, id: string, updates: Partial<Loan>): Promise<void> {
  await ensureConnection()
